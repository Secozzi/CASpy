...
from caspy3.qt_assets.tabs.simplify import SimpWorker
...
class PrevSimpExpTester(BaseTester):
    ...
    @BaseTester.call_worker(SimpWorker)
    ...
    def test_prev_simp_exp(self):
        ...
        solution = {
            "simp": ["   2         2   \nsin (x) + cos (x)", 0],
            "latex": "\\sin^{2}{\\left(x \\right)} + \\cos^{2}{\\left(x \\right)}",
        }
        return command, params, solution
    ...
    def test_prev_simp_exp_latex(self):
        ...
        solution = {
            "simp": ["\\sin^{2}{\\left(x \\right)} + \\cos^{2}{\\left(x \\right)}", 0],
            "latex": "\\sin^{2}{\\left(x \\right)} + \\cos^{2}{\\left(x \\right)}",
        }
        return command, params, solution
