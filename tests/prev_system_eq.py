...
class PrevSystemEqTester(BaseTester):
    ...
    def test_prev_system_eq_too_many_equal(self):
        command = "prev_system_eq"
        params = [['x+y = 5', 'x**2+y**2 = 17 = 2'], 'x y', 'Complexes', 1, 1, False, False]
        solution = {'error': ["Error: \nEnter only one '=' on line 2"]}
        return command, params, solution
    ...
    def test_prev_system_eq_ode(self):
        command = "prev_system_eq"
        params = [['f\'(x) = f(x)*g(x)*sin(x)', 'g\'(x) = g(x)**2*sin(x)'], 'f(x) g(x)', 'Complexes', 2, 1, False, False]
        solution = {'eq': ['Domain: S.Complexes\n\nd                          \n--(f(x)) = f(x)*g(x)*sin(x)\ndx                         \n\nd           2          \n--(g(x)) = g (x)*sin(x)\ndx                     \n\nVariables to solve for: [f(x), g(x)]', 0], 'latex': '\\frac{d}{d x} f{\\left(x \\right)} = f{\\left(x \\right)} g{\\left(x \\right)} \\sin{\\left(x \\right)} \\ \\frac{d}{d x} g{\\left(x \\right)} = g^{2}{\\left(x \\right)} \\sin{\\left(x \\right)}'}
        return command, params, solution
    ...
    def test_prev_system_eq_unicode(self):
        command = "prev_system_eq"
        params = [['exp(x-y) = 2', 'x**2+y**2 = 17'], 'x y', 'Complexes', 1, 1, True, False]
